...
class CreatePostModal extends Component<any, any> {
    ...
    handleSubmit() {
        if (!this.state.content.trim() || this.state.isSubmitting) return;

        this.setState({ isSubmitting: true });

        // Dispatch legacy thunk action
        this.props.createThread(this.state.content, this.props.currentUser?.id || 'user_1');

        // Fake network delay for smooth UI feedback mapping to the thunk delay
        setTimeout(() => {
            this.setState({ isSubmitting: false, content: '' });
            this.props.onClose();
        }, 400);
    }
        ...
        return (
            <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4 animate-fade-in transition-all">
                ...
                >
                    ...
                    <div className="flex-1 overflow-y-auto p-4 flex gap-4 disabled:opacity-50">
...
export default connect(mapStateToProps, { createThread })(CreatePostModal);
